...
public class YutaBeatMinecraftTask extends Task {
    ...
    private  Item[] _ImportantItems = {Items.SHIELD, Items.IRON_SWORD, Items.STONE_PICKAXE, Items.STONE_AXE, Items.WOODEN_PICKAXE};
    ...
    private boolean getChest = false;
    private boolean placingChest = false;
    private final TimerGame bedPlace = new TimerGame(3);
    ...
    protected Task onTick(AltoClef mod) {
      ...
      //  BlockPos playerPos = getPlayer().getBlockPos();
        ...
        //   if (!hasGoodWeapon && !hasProtection && !HasBackupPickaxe && !HasGoodAxe && !HasGoodPickaxe) {
        ...
        // }
        /*if (getInstance().crosshairTarget == EndermanEntity && mod.getItemStorage().hasItem(Items.ENDER_EYE))
        ...
        if (!(WorldHelper.getCurrentDimension() == Dimension.END)) {
            ...
            if (resourceGet && mod.getItemStorage().hasEmptyInventorySlot() && !tooMuchShit && !WorldHelper.canSleep())
            {
                if (!mod.getItemStorage().hasItem(Items.WOODEN_SHOVEL)) {
                    setDebugState("Getting Materials... but getting a Shovel, just in case. (" + (80 - getMaterialTimer.getDuration()) + "seconds)");
...
//if ()
            if (!WorldHelper.canSleep() && sleepNow)
                sleepNow = false;
            if (WorldHelper.canSleep() && WorldHelper.getCurrentDimension() == Dimension.OVERWORLD && mod.getItemStorage().hasItem(ItemHelper.BED))
            {
                sleepNow = true;
            ...
            if (!hasStartedTheSpeedrun) {
                if (!mod.getItemStorage().hasItem(Items.CHEST)) {
                    ...
                    Debug.logMessage("Done getting chests for potential Inventory clutter management.");
    ...
    if (mod.getItemStorage().hasItem(Items.CHEST)) {
        ...
        return new PlaceBlockNearbyTask(Block.getBlockFromItem(Items.CHEST));
            ...
            if (!mod.getItemStorage().hasEmptyInventorySlot() || tooMuchShit) {
                // Check if the current inventory is different from the previously stored BloatedInventory
            ...
            if (tooMuchShit) {
                ...
                //    if (mod.getItemStorage().hasItem(Items.CHEST) && mod.getBlockTracker().getNearestTracking().BlockPos) {
//
  //                  }
                    ...
                    // If there are no items remaining from BloatedInventory, resume normal functions
                    tooMuchShit = false;
                    BloatedInventory = null;
                    hasStartedTheSpeedrun = false;
                ...
                if (!mod.getItemStorage().hasItem(Items.STONE_SWORD) && !mod.getItemStorage().hasItem(Items.IRON_SWORD) && !mod.getItemStorage().hasItem(Items.DIAMOND_SWORD)) {
            ...
            if (!HasGoodPickaxe && !mod.getItemStorage().hasItem(Items.IRON_PICKAXE) && !mod.getItemStorage().hasItem(Items.DIAMOND_PICKAXE)) {
